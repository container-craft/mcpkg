...
static McPkgInfoEntry *mcpkg_info_entry_from_cjson(cJSON *mod_item)
{
	...
	if (cJSON_IsString(temp_json)) entry->description = strdup(
	...
	if (cJSON_IsNumber(temp_json)) entry->downloads = (uint32_t)
		                temp_json->valuedouble;

	temp_json = cJSON_GetObjectItemCaseSensitive(mod_item, "date_modified");
	if (cJSON_IsString(temp_json)) entry->date_modified = strdup(
		                        temp_json->valuestring);

	temp_json = cJSON_GetObjectItemCaseSensitive(mod_item, "latest_version");
	if (cJSON_IsString(temp_json)) entry->latest_version = strdup(
		                        temp_json->valuestring);
	...
	if (cJSON_IsString(temp_json)) entry->client_side = strdup(
		                        temp_json->valuestring);

	temp_json = cJSON_GetObjectItemCaseSensitive(mod_item, "server_side");
	if (cJSON_IsString(temp_json)) entry->server_side = strdup(
		                        temp_json->valuestring);
...
MCPKG_ERROR_TYPE modrith_client_update(ModrithApiClient *client_data)
{
	...
	while (true) {
		char facets_raw[256];
		snprintf(facets_raw, sizeof(facets_raw),
		         "[[\"categories:%s\"],[\"versions:%s\"]]",
		         mcpkg_modloader_str(client_data->mod_loader),
		         client_data->mc_version);
	...
	snprintf(package_info_path, sizeof(package_info_path), "%s/Packages.info",
	         cache_base_path);
	...
	if (mcpkg_fs_compressed_file(compressed_path, sbuf_all_mods.data,
	                             sbuf_all_mods.size) != MCPKG_ERROR_SUCCESS) {
                ...
                VisitedSet *visited)
{
	...
	rc = mcpkg_get_db_append_single(client, mods_dir, install_db,
	                                entry); // takes ownership via upsert
	if (rc != MCPKG_ERROR_SUCCESS) {
		mcpkg_entry_free(entry);
		return rc;
	}
...
int modrith_version_to_entry(cJSON *v, McPkgEntry **out)
{
	...
	if (cJSON_IsString(tmp)) e->name = strdup(
		                tmp->valuestring); // human-readable version name
                                        ...
                                        const char *id_or_slug)
{
	...
	if (rc != MCPKG_ERROR_SUCCESS) {
		fprintf(stderr,
		        "Warning: search cache not available for %s/%s (continuing install anyway)\n",
		        loader, mcver);
