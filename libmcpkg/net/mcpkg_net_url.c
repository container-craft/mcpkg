...
MCPKG_API int mcpkg_net_url_parse(McPkgNetUrl *u, const char *url_utf8)
{
	...
	return curlu2neterr(curl_url_set(u->h, CURLUPART_URL, url_utf8,
	                                 CURLU_GUESS_SCHEME));
}

/* ---------- helpers ---------- */

static int part_to_buf(CURLU *h, CURLUPart part, unsigned flags, char *buf,
                       size_t buf_sz)
{
	char *tmp = NULL;
	CURLUcode cu;
...
MCPKG_API int mcpkg_net_url_is_empty(McPkgNetUrl *u, int *out_is_empty)
{
	...
	if (has_part(u->h, CURLUPART_SCHEME, 0,
	             &yes) != MCPKG_NET_NO_ERROR) return MCPKG_NET_ERR_PROTO;
	if (yes) {
		*out_is_empty = 0;
		return MCPKG_NET_NO_ERROR;
	}

	if (has_part(u->h, CURLUPART_HOST, 0,
	             &yes) != MCPKG_NET_NO_ERROR) return MCPKG_NET_ERR_PROTO;
	if (yes) {
		*out_is_empty = 0;
		return MCPKG_NET_NO_ERROR;
	...
	if (part_to_buf(u->h, CURLUPART_PATH, 0, path,
	                sizeof(path)) != MCPKG_NET_NO_ERROR) return MCPKG_NET_ERR_PROTO;
	if (path[0] && !(path[0] == '/' && path[1] == '\0')) {
		*out_is_empty = 0;
		return MCPKG_NET_NO_ERROR;
	}

	if (has_part(u->h, CURLUPART_QUERY, 0,
	             &yes) != MCPKG_NET_NO_ERROR) return MCPKG_NET_ERR_PROTO;
	if (yes) {
		*out_is_empty = 0;
		return MCPKG_NET_NO_ERROR;
	...
	if (has_part(u->h, CURLUPART_FRAGMENT, 0,
	             &yes) != MCPKG_NET_NO_ERROR) return MCPKG_NET_ERR_PROTO;
...
MCPKG_API int mcpkg_net_url_port(McPkgNetUrl *u, int *out_port)
{
	...
	if (part_to_buf(u->h, CURLUPART_PORT, 0, tmp,
	                sizeof(tmp)) != MCPKG_NET_NO_ERROR) return MCPKG_NET_ERR_PROTO;
	if (!tmp[0]) {
		*out_port = 0;
		return MCPKG_NET_NO_ERROR;
	}
...
static int set_part(McPkgNetUrl *u, CURLUPart part, const char *val,
                    unsigned flags)
{
	if (!u || !u->h) return MCPKG_NET_ERR_INVALID;
                                      ...
                                      const char *val_utf8)
{
	...
	{
		CURLUcode cu = curl_url_set(u->h, CURLUPART_QUERY, kv,
