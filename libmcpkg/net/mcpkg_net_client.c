...
static int ensure_curl(struct McPkgNetClient *c)
{
    ...
    if (ret == MCPKG_NET_NO_ERROR && !c->curl) {
        ...
        if (!h) {
            ...
            curl_easy_setopt(h, CURLOPT_USERAGENT, c->user_agent ? c->user_agent : "mcpkg/unknown");
            curl_easy_setopt(h, CURLOPT_CONNECTTIMEOUT_MS, c->connect_timeout_ms > 0 ? c->connect_timeout_ms : 10000L);
            curl_easy_setopt(h, CURLOPT_TIMEOUT_MS, c->total_timeout_ms > 0 ? c->total_timeout_ms : 30000L);
...
static int load_offline_file(const char *root, const char *path, struct McPkgNetBuf *out_body)
{
    int ret = MCPKG_NET_NO_ERROR;
    FILE *fp = NULL;
    char *full = NULL;
...
{
    (void)c; (void)n; /* future extension */
                  ...
                  long *out_http_code)
{
    ...
    if (ret == MCPKG_NET_NO_ERROR && c->offline) {
        out_body->len = 0;
        ret = load_offline_file(c->offline_root ? c->offline_root : ".", path, out_body);
