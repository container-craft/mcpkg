...
int mtx_trylock(mtx_t *mtx)
{
	...
	if (!mtx->mTimed) {
		...
		ret = (WaitForSingleObject(mtx->mHandle.mut,
		                           0) == WAIT_OBJECT_0) ? thrd_success : thrd_busy;
	}
...
int cnd_timedwait(cnd_t *cond, mtx_t *mtx, const struct timespec *ts)
{
	...
	if (timespec_get(&now, TIME_UTC) == TIME_UTC) {
		unsigned long long nowInMilliseconds = now.tv_sec * 1000 + now.tv_nsec /
		                                       1000000;
		unsigned long long tsInMilliseconds  = ts->tv_sec * 1000 + ts->tv_nsec /
		                                       1000000;
...
static void _tinycthread_tss_cleanup(void)
{
	...
	for (iteration = 0 ; iteration < TSS_DTOR_ITERATIONS
	     && again > 0 ; iteration++) {
		again = 0;
...
int thrd_create(thrd_t *thr, thrd_start_t func, void *arg)
{
	...
	_thread_start_info *ti = (_thread_start_info *)malloc(sizeof(
	                                 _thread_start_info));
	if (ti == NULL) {
		return thrd_nomem;
	}
