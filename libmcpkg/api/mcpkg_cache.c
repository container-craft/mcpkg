...
static int zstd_decompress_dynamic(const void *src, size_t srcSize, char **out,
                                   size_t *outSize)
{
...
static int decompress_zstd_file_buffer(const char *inBuf, size_t inSize,
...
static int unpack_all_mods_from_buffer(McPkgCache *cache, const char *buffer,
                                       size_t size)
{
	...
	while (msgpack_unpack_next(&unpacked, buffer, size, &offset)) {
		...
		if (temp_count == temp_capacity) {
			...
			McPkgInfoEntry **tmp = (McPkgInfoEntry **)realloc(temp_mods,
			                       new_cap * sizeof(*tmp));
...
void mcpkg_cache_free(McPkgCache *cache)
{
	...
	if (cache->mods) {
		for (size_t i = 0; i < cache->mods_count;
		     ++i) mcpkg_info_entry_free(cache->mods[i]);
		free(cache->mods);
	}
	free(cache);
}

int mcpkg_cache_load(McPkgCache *cache, const char *mod_loader,
                     const char *version)
{
	...
	free(cache->base_path);
	cache->base_path = NULL;
	if (asprintf(&cache->base_path, "%s/%s/%s/%s", cache_root, mod_loader, codename,
	             version) < 0 || !cache->base_path) {
		rc = MCPKG_ERROR_GENERAL;
		goto done;
	}

	/* Build full paths */
	if (asprintf(&compressed_path,   "%s/Packages.info.zstd", cache->base_path) < 0
	    || !compressed_path)
		goto done;
	if (asprintf(&uncompressed_path, "%s/Packages.info",      cache->base_path) < 0
	    || !uncompressed_path)
		goto done;
...
McPkgInfoEntry **mcpkg_cache_search(McPkgCache *cache, const char *package,
