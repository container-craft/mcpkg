                                   ...
                                   str_array *packages)
{
	...
	ModrithApiClient *client = modrith_client_new(mc_version,
	                           mcpkg_modloader_from_str(mod_loader));
	if (!client) {
	...
	for (size_t i = 0; i < packages->count; ++i) {
		...
		char *installed_ver = (char *)mcpkg_get_find_installed_version(cache_root,
		                      mod_loader, mc_version, codename, pkg);
		if (installed_ver) {
			// Ask Modrinth for the best candidate to compare; if same, skip
			cJSON *versions = modrith_get_versions_json(client, pkg);
			if (!versions) {
				fprintf(stderr,
				        "   %s: failed to query versions; proceeding to install anyway\n", pkg);
...
int mcpkg_get_remove(const char *mc_version, const char *mod_loader,
                     str_array *packages)
{
	if (!mc_version || !mod_loader || !packages || packages->count == 0)
		return MCPKG_ERROR_PARSE;
	...
	if (kept == 0) {
		...
		rc = mcpkg_get_db_write_all(install_db, entries,
		                            kept) == 0 ? MCPKG_ERROR_SUCCESS : MCPKG_ERROR_FS;
	}
                       ...
                       ...)
{
	...
	while (1) {
		va_start(ap, fmt);
		int need = vsnprintf((*buf) ? *buf + *len : NULL, (*buf
		                     && *cap > *len) ? *cap - *len : 0, fmt, ap);
		va_end(ap);
		if (need < 0)
			return -1;
                       ...
                       const char *mod_loader)
{
	...
	// Paths
	char *install_db = NULL;
	if (mcpkg_fs_db_dir(&install_db, cache_root, mod_loader, codename,
	                    mc_version) != 0)
		return MCPKG_ERROR_OOM;
