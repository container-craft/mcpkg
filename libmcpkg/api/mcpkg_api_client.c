                ...
                const char *ua)
{
	...
	if (curl_easy_setopt(client->curl, CURLOPT_USERAGENT,
	                     client->user_agent) == CURLE_OK)
		return MCPKG_ERROR_SUCCESS;
	...
	for (int attempt = 0; attempt < max_attempts; ++attempt) {
		...
		if (rc != CURLE_OK) {
			...
			if (client->ratelimit_reset > 0) {
				...
				long delta_s = client->ratelimit_reset > now ? (client->ratelimit_reset - now) :
				               client->ratelimit_reset;
				if (delta_s < 0)
					delta_s = 1;
