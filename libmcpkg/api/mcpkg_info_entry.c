...
{
    if (!obj || obj->type != MSGPACK_OBJECT_MAP || !entry)
        return 1;
    ...
    for (size_t i = 0; i < map.size; ++i) {
        ...
        if (equal_key(key, "id")){
            if (val.type == MSGPACK_OBJECT_STR)
                entry->id = strndup_safe(val.via.str.ptr, val.via.str.size);

        }else if (equal_key(key, "name")){
            if (val.type == MSGPACK_OBJECT_STR)
                entry->name = strndup_safe(val.via.str.ptr, val.via.str.size);

        }else if (equal_key(key, "author")){
            if (val.type == MSGPACK_OBJECT_STR)
                entry->author = strndup_safe(val.via.str.ptr, val.via.str.size);

        } else if (equal_key(key, "title")) {
            if (val.type == MSGPACK_OBJECT_STR)
                entry->title = strndup_safe(val.via.str.ptr, val.via.str.size);

        } else if (equal_key(key, "description")) {
            if (val.type == MSGPACK_OBJECT_STR)
                entry->description = strndup_safe(val.via.str.ptr, val.via.str.size);

        } else if (equal_key(key, "icon_url")) {
            if (val.type == MSGPACK_OBJECT_STR)
                entry->icon_url = strndup_safe(val.via.str.ptr, val.via.str.size);
            ...
            if (val.type == MSGPACK_OBJECT_STR)
                entry->date_modified = strndup_safe(val.via.str.ptr, val.via.str.size);

        } else if (equal_key(key, "latest_version")) {
            if (val.type == MSGPACK_OBJECT_STR)
                entry->latest_version = strndup_safe(val.via.str.ptr, val.via.str.size);

        } else if (equal_key(key, "license")) {
            if (val.type == MSGPACK_OBJECT_STR)
                entry->license = strndup_safe(val.via.str.ptr, val.via.str.size);

        } else if (equal_key(key, "client_side")) {
            if (val.type == MSGPACK_OBJECT_STR)
                entry->client_side = strndup_safe(val.via.str.ptr, val.via.str.size);

        } else if (equal_key(key, "server_side")) {
            if (val.type == MSGPACK_OBJECT_STR)
                entry->server_side = strndup_safe(val.via.str.ptr, val.via.str.size);
        }
    }
...
char *mcpkg_info_entry_to_string(const McPkgInfoEntry *entry)
{
    ...
    asprintf(&output,
             ...
             mcpkg_safe_str(entry->title),
             mcpkg_safe_str(entry->description),
             entry->downloads,
             mcpkg_safe_str(categories_str),
             mcpkg_safe_str(versions_str),
             mcpkg_safe_str(entry->date_modified),
             mcpkg_safe_str(entry->latest_version),
             mcpkg_safe_str(entry->license),
             mcpkg_safe_str(entry->client_side),
             mcpkg_safe_str(entry->server_side)
    );
