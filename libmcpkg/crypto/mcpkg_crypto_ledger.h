...
/* ---- simple byte arrays ---- */
typedef struct { uint8_t b[MCPKG_PUBKEY_LEN]; } McPkgPubKey32;
typedef struct { uint8_t b[MCPKG_SIG_LEN];    } McPkgSig64;
typedef struct { uint8_t b[MCPKG_HASH32_LEN]; } McPkgHash32;
...
typedef struct McPkgRevoke {
    ...
    McPkgHash32         target_hash;  /* when kind=KEY or ATTESTATION; for PACKAGE, pack separately in MP */
