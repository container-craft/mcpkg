...
{
	const char *mc_version = getenv(ENV_MC_VERSION) ? getenv(
	                                 ENV_MC_VERSION) : "1.21.8";
	const char *mod_loader = getenv(ENV_MC_LOADER) ? getenv(
	                                 ENV_MC_LOADER) : "fabric";
	int opt;

	// Parse global options first
	int option_index = 0;
	while ((opt = getopt_long(argc, argv, "v:l:h", long_options,
	                          &option_index)) != -1) {
		switch (opt) {
			case 'v':
				mc_version = optarg;
